...
init([Group, Topic, Partition]) ->
    ...
    {ok, catch_up, #state{group=Group, topic=Topic, partition=Partition, next_offset_to_send_from=0}}.

catch_up(enter, _OldState, _State) ->
    {keep_state_and_data, {state_timeout, 0, send_batch}};
catch_up(state_timeout, send_batch, #state{group=_Group, topic=Topic, partition=Partition,  next_offset_to_send_from=Offset}=State) ->
    Rows = esque_qs:all_messages_from_offset(Topic, Partition, Offset),
    send_batch(Rows),
    {next_state, listen, State}.   
listen(enter, _OldState, _State) ->
    keep_state_and_data;
listen(cast, {send, Msgs} , #state{group=Group, topic=Topic, partition=Partition}=_State) ->
